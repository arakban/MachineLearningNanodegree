...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing == True:
            self.alpha, self.epsilon = 0.0 , 0.0
        
        else: 
            self.epsilon = math.exp(-self.a*self.tCount)
            self.tCount += 1
    ...
    def choose_action(self, state):
        ...
        if (not self.learning) or (random.random() < self.epsilon):
            ...
            for act, QVal in self.Q[state].iteritems():
                if QVal == maxQValue:
                    maxQValueActions.append(act)
            
            action = random.choice(maxQValueActions)
...
def run():
    ...
    agent = env.create_agent(LearningAgent, learning = True, alpha = 0.45)
